...
train_g.add_arg("batch_size",        int,    512,                   "Batch size.")#1024
...
train_g.add_arg("save_steps",          int,   20,                  "save_steps")#20
train_g.add_arg("num_workers",      int,    0,                     "num_workers")

log_g = ArgumentGroup(parser, "logging", "logging related.")
log_g.add_arg("skip_steps",          int,    100,    "Step intervals to print loss.")
log_g.add_arg("start_evaluation_epochs",      int,    10, "epoch index to start evaluation")
log_g.add_arg("interval_evaluation_epochs",         int,   1,      "epoch interval for evaluation")
...
# parser.add_argument("--dataset",       default="starqe",   type=str)
# parser.add_argument("--max_dataset_len", default=1000000, type=int)
# parser.add_argument("--train_tasks", default="*", type=str)
# parser.add_argument("--validation_tasks", default="1hop", type=str)
# parser.add_argument("--valid_eval_tasks", default="1hop", type=str)
# parser.add_argument("--test_tasks", default="1hop", type=str)
# parser.add_argument("--prediction_tasks", default="1hop, 2hop, 3hop, 2i, 3i, 1hop-2i, 2i-1hop", type=str)
...
parser.add_argument("--prediction_tasks", default="1p, 2cp, 2i, 2in, 2p, 2u, 3cp, 3i, 3in, 3p, inp, ip, pi, pin, pni, up", type=str)

parser.add_argument("--train_shuffle", default=True, type=bool)
parser.add_argument("--prediction_ckpt", default="ckptsstarqe-best-valid-DIM256.ckpt", type=str)
...
def main(args):
    ...
    if config["do_learn"]:
        ...
        for run_type in learning_run_type_ls:
            task_str = config[run_type+"_tasks"]
            task_filter = [task.strip() for task in task_str.split(",")]
        ...
        for epoch in training_range:          
            ...
            if epoch % config["interval_evaluation_epochs"] == 0 and epoch >= config["start_evaluation_epochs"]:
                ...
                print_evaluation(
                    gran_model, 
                    learning_dataloader_ls["test"], 
                    learning_all_run_gt_dict["test"], 
                    device, 
                    writer_dir["test"], 
                    writer_dir["test"],
                    valid_score_task_filter,
    ...
    if config["do_predict"]:
        ...
        for run_type in prediction_run_type_ls:
            task_str = config["prediction_tasks"]
            task_filter = [task.strip() for task in task_str.split(",")]
...
def print_evaluation(gran_model, eval_dataloader, single_run_gt_dict, device, writer_dir, log_prefix, eval_result_tasks_filter):
    ...
    for task in eval_metrics:
        format_str += task  
        for metric_type in eval_metrics[task]:
            format_str += "\t%.4f" % (eval_metrics[task][metric_type])
        format_str += "\n"         
